...
<head>
<script>
var appRoot = '${root}'; // contextRoot를 root라는 이름으로 저장.
var bno = ${board.bno}; // board안의 bno를 저장. 
</script>
<meta charset="UTF-8">
<link rel="stylesheet"
...
4번 글에 새로운 reply를 달고 create 테스트 console에 결과도출 */ 
/*
...
<script>
	...
	$(document).ready(function() {
		...
		function dateString(date) {
			var d = new Date(date);//d 라는 이름으로 저장.
			return d.toISOString().split("T")[0]; 
		}
		//d를 to string해서 T타입으로 뿌리기
		
		// 댓글 목록 가져오기 함수
		function showList() {
			replyService.getList({bno: bno}, function(list) {
				// console.log(list);   getList list 번호, 작동
				
				
				var replyUL = $("#reply-ul");//replyUL 객체 생성.
				replyUL.empty();// 한번 비워주고 
		...
		$("#reply-ul").on("click", "li", function() {
			...
			//댓글 목록 클릭시 콘솔에는 기능한 함수 데이터의 rno를 띄운다.
			// 하나의 댓글 읽어오기
			...
			// get함수를 통해 기존에 가지고 있던 input을 띄우고 
		...
		$("#reply-modify-button").click(function() {
			...
			replyService.update(data, function() {
				...
				showList();
			}); // 수정 버튼 클릭시 작성자는 고정, 댓글 내용만 수정해서 닫기 가능
		});
		...
		$("#reply-delete-button").click(function() {
			...
			replyService.remove(rno, function() {
				...
				$("#modify-reply-modal").modal('hide');
				showList();
			}); //삭제 버튼을 통해 클릭시 rno를 input2로 받아서 저장. remove 실행. 
		});
